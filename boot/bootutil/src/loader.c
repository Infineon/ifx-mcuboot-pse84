 ...
 * decrypted when copied in ram */
...
#if defined(MCUBOOT_ENC_IMAGES_XIP_MULTI) || defined(MCUBOOT_ENC_IMAGES_XIP_MULTI_KEY_MULTY) || defined(MCUBOOT_USE_ENC_IFX_SE)
    if (MUST_DECRYPT(fap, image_index, hdr)) {
#else
    if (MUST_DECRYPT(fap, image_index, hdr) && !IS_RAM_BOOTABLE(hdr)) {
#endif /* defined(MCUBOOT_ENC_IMAGES_XIP_MULTI) || defined(MCUBOOT_ENC_IMAGES_XIP_MULTI_KEY_MULTY) || defined(MCUBOOT_USE_ENC_IFX_SE) */
    ...
    FIH_UCALL(bootutil_ifx_se_img_validate, fih_complex_result, \
              BOOT_CURR_ENC(state), image_index, hdr, fap,      \
              tmpbuf, BOOT_TMPBUF_SZ, NULL, 0);

    BOOT_LOG_DBG(" * bootutil_ifx_se_img_validate expected = 0x%x, " \
                 "returned = 0x%x",                                  \
                 ...
                 uint32_t off_src, uint32_t off_dst, uint32_t sz)
{
    ...
    int chunk_sz;
    int rc;
#if (defined (MCUBOOT_ENC_IMAGES) && defined(MCUBOOT_USE_ENC_IFX_SE))
    /* EPC4 encryption flow */
    (void)state;
#else
#if (defined (MCUBOOT_ENC_IMAGES) && !defined(MCUBOOT_ENC_IMAGES_XIP)) || \
    (defined (MCUBOOT_ENC_IMAGES) && defined(MCUBOOT_ENC_IMAGES_XIP) && \
    (defined(MCUBOOT_ENC_IMAGES_XIP_MULTI)))
    uint32_t off;
    uint32_t tlv_off;
    size_t blk_off;
    struct image_header *hdr;
    ...
    uint8_t image_index;
          ...
          (defined (MCUBOOT_ENC_IMAGES) && defined(MCUBOOT_ENC_IMAGES_XIP) && \
          (defined(MCUBOOT_ENC_IMAGES_XIP_MULTI))) */
#endif /* (defined (MCUBOOT_ENC_IMAGES) && defined(MCUBOOT_USE_ENC_IFX_SE)) */
...
#endif /* MCUBOOT_PLATFORM_CHUNK_SIZE */
...
#endif /* defined(MCUBOOT_WATCHDOG_ENABLED) */
    }
...
boot_copy_image(struct boot_loader_state *state, struct boot_status *bs)
{
    ...
    BOOT_LOG_DBG(" * primary slot sectors: %u", (unsigned)sect_count);
...
context_boot_go_flash(struct boot_loader_state *state, struct boot_rsp *rsp)
{
    ...
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        ...
        (void)memset(rsp, 0, sizeof(struct boot_rsp));
        (void)memset(&copy_rsp, 0, sizeof(struct boot_rsp));
        ...
        if (0 == BOOT_CURR_IMG(state)) {
            ...
            if (IS_ENCRYPTED(boot_img_hdr(state, BOOT_PRIMARY_SLOT)))
            {
                ...
                (void)memcpy((uint8_t*)rsp->xip_key, bs.enckey[BOOT_PRIMARY_SLOT], BOOTUTIL_CRYPTO_AES_CTR_KEY_SIZE);
                (void)memcpy((uint8_t*)copy_rsp.xip_iv, BOOT_CURR_ENC(state)->aes_iv, BOOTUTIL_CRYPTO_AES_CTR_BLOCK_SIZE);
                (void)memcpy((uint8_t*)copy_rsp.xip_key, bs.enckey[BOOT_PRIMARY_SLOT], BOOTUTIL_CRYPTO_AES_CTR_KEY_SIZE);
            }
#if defined(MCUBOOT_ENC_IMAGES_XIP_MULTI)
        }
#endif /* MCUBOOT_ENC_IMAGES_XIP_MULTI */
    ...
    fill_rsp(state, rsp);
    fill_rsp(state, &copy_rsp);

    if (memcmp(rsp, &copy_rsp, sizeof(struct boot_rsp)) != 0)
    {
        fih_rc = FIH_FAILURE;
        goto out;
    }
                                    ...
                                    uint32_t src_sz, uint32_t img_dst)
{
    ...
    uint32_t max_sz = IFX_ENC_BLOCK_SIZE;
#else
...
#else
    (void)image_index;
    /* Header and TLV are not encrypted.
     * Copy Header to RAM */
...
#if defined(MCUBOOT_VALIDATE_PRIMARY_SLOT) || defined(MCUBOOT_DEPENDENCY_CHECK)
/**
...
#endif /* defined(MCUBOOT_VALIDATE_PRIMARY_SLOT) || defined(MCUBOOT_DEPENDENCY_CHECK) */
...
boot_switch_to_sram_image(struct boot_loader_state *state, uint32_t slot)
{
    ...
    if (rc == 0) {
        ...
        /* Image is already decrypted, so clear encrypted flags in the header */
...
#endif /* defined(MCUBOOT_DIRECT_XIP_REVERT) */
...
boot_go_for_image_id(struct boot_rsp *rsp, uint32_t image_id)
{
    fih_int fih_rc = FIH_FAILURE;
#if BOOT_IMAGE_NUMBER > 1
    bool copy_img_mask[BOOT_IMAGE_NUMBER];
#endif
    ...
    (void)memcpy(copy_img_mask, boot_data.img_mask, BOOT_IMAGE_NUMBER);
#endif

    FIH_CALL(context_boot_go_flash, fih_rc, &boot_data, rsp);

#if defined(MCUBOOT_USE_ENC_IFX_SE)
    /* Zeroize encryption state for the selected image */
    boot_enc_zeroize((boot_data.enc[image_id]));
#endif /* MCUBOOT_USE_ENC_IFX_SE */

#if BOOT_IMAGE_NUMBER > 1
    /* Ensure the image mask was not changed unexpectedly */
    if (memcmp(boot_data.img_mask, copy_img_mask, BOOT_IMAGE_NUMBER) != 0)
    {
        fih_rc = FIH_FAILURE;
    }
#endif
...
int
boot_get_image_version(uint32_t image_id, uint32_t slot_id, struct image_version* image_version)
{
    int rc = -1;
    struct boot_status bs = {0};
    ...
    if (image_id < BOOT_IMAGE_NUMBER) {
        ...
        if (boot_read_image_headers(&boot_data, false, &bs) == 0) {
            *image_version = boot_img_hdr(&boot_data, slot_id)->ih_ver;
            rc = 0;
        }
    }
