...
#define IMAGE_TLV_ENV_KEY_ID        (0x78)   /* Provisioned custom key ID for EPC4 encryption */
					   /*
...
#define CLEAR_ENCRYPT_FLAGS(hdr) ((hdr)->ih_flags &= ~((IMAGE_F_ENCRYPTED_AES128 | IMAGE_F_ENCRYPTED_AES256)))
#define SET_ENCRYPT_FLAGS(hdr) ((hdr)->ih_flags |= (IMAGE_F_ENCRYPTED_AES128 | IMAGE_F_ENCRYPTED_AES256))

#if defined(MCUBOOT_ENC_IMAGES_XIP) || defined(MCUBOOT_USE_ENC_IFX_SE)
...
#define SECURITY_BIT                     (28u)
#define IS_RAM_BOOTABLE_SECURE(hdr)                                       \
    ((((hdr)->ih_flags & IMAGE_F_RAM_LOAD) == IMAGE_F_RAM_LOAD) && \
     ((hdr)->ih_load_addr != 0U) && ((hdr)->ih_load_addr != (uintptr_t)(-1)) && \
     ((hdr)->ih_load_addr & (1<<SECURITY_BIT)) != 0)
#else
